...
| **Cloudflare Tunnels** | `jellyfin.rickermedia.com`, `node-main.rickermedia.com`, `cinemafred.com` — no inbound firewall ports opened |

Docmost is retired from the desired configuration; its notes were migrated
to the local ML Obsidian vault. Its database and uploads are retained for
recovery. See [Docmost retirement](retire-docmost.md).
